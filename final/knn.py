...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    # TODO: Fill dists[i_test][i_train]
                    dists[i_test][i_train] = np.sum(sqiare(self.train_X[i_train] - X[i_test]))
        return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                # TODO: Fill the whole row of dists[i_test]
                # without additional loops or list comprehensions
                dists[i_test, :] = np.sum(np.square(X[i_test] - self.train_X), axis=1)
        return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        if self.metric == 'euclidean':
            ...
            train_X_squared = np.sum(self.train_X**2, axis=1)
            dot = np.dot(X, self.train_X.T)
            dists = np.sqrt(X_squared - (2 * dot) + train_X_squared)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            closest = self.train_y[k_nearest_idxs]
